...
export default function Navbar({ products = [] }) {
  ...
  // ─── Lógica Dinámica del Menú (Memoizada) ───
  // Construye el menú basándose en los productos reales que recibe de Astro
  const menuTree = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      // Guardamos el primer producto "featured" que encontremos para la tarjeta promocional
      if (p.featured && !tree[mainCatId].featuredProduct) {
        tree[mainCatId].featuredProduct = p;
      }
    });

    // Convertimos los Sets a Arrays y ordenamos alfabéticamente
  ...
  // Obtenemos los datos de la categoría activa para renderizar el panel desplegable
        ...
        <div className="max-w-[1320px] mx-auto px-5 md:px-8 h-full flex items-center gap-4 lg:gap-8">
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0 ml-auto md:ml-0">
            ...
            <button
              ...
              aria-label={
                isMounted
                  ? `Shopping cart, ${currentCartCount} item${currentCartCount !== 1 ? "s" : ""}`
                  : "Shopping cart"
              }
            >
              <svg
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                  {activeCategoryData.subcategories.map((sub) => (
                    <a
                      key={sub}
                      href={`/category/${activeMenu}?sub=${encodeURIComponent(sub.toLowerCase())}`}
                    ...
                    >
                      <span className="text-[10px] text-watermelon/50 group-hover:text-watermelon f2-transition">
                        ✦
                      </span>
                      {sub}
                    </a>
